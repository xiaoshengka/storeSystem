...
WRITE_WORKLOADS = {"hash-insert", "hash-mixed", "zset-insert", "zset-mixed"}
CSV_FIELDS = (
...
def build_cases(targets, policies, workloads, repeats):
        ...
        for workload in selected:
            if workload in HASH_WORKLOADS:
                ...
                for target in local_targets:
                    for repeat in range(1, repeats + 1):
                        cases.append((target, target, policy, workload, repeat))
            if "redis" in targets:
                for repeat in range(1, repeats + 1):
                    cases.append(("redis", "redis", policy, workload, repeat))
    return cases
...
def main() -> int:
    ...
    if any(value not in ALL_WORKLOADS for value in workloads):
    ...
    if (args.repeats < 1 or args.connections < 1 or args.requests < 1 or
            args.warmup < 0 or args.pipeline < 1 or args.keyspace < 10 or
            args.timeout <= 0 or args.connections > 1024 or
            args.pipeline > 1024 or args.requests < args.connections):
    ...
    cases = build_cases(targets, policies, workloads, args.repeats)
    if not cases:
    ...
    print(
        f"dataset: one shared Hash/ZSet key; read/mixed keyspace={args.keyspace}; "
        f"insert starts empty and grows to requests={args.requests}; "
    ...
    with tempfile.TemporaryDirectory(prefix="collection-compare-") as directory:
        ...
        with output.open("w", newline="") as stream:
            ...
            for run_number, (target, server_engine, policy, workload,
                ...
                aof_path = temp / f"{target}-{policy}-{workload}-{repeat}.aof"
                ...
                try:
                    ...
                    result = subprocess.run([
                        ...
                        "-w", str(args.warmup), "-P", str(args.pipeline),
                    ...
                    cardinality_command = (b"HLEN" if workload.startswith("h")
                                           else b"ZCARD")
                ...
                row = {
                    ...
                    "pipeline": args.pipeline, "requests": args.requests,
    ...
    print(f"completed {len(cases)} runs; wrote {output}", flush=True)
    return 0
